...
export const AuthProvider = ({ children }) => {
    ...
    const signUpAuth = async (email, password, name, status, safeguarding, DEFAULT_PFP) => {
        setIsLoading(true)
        try {
            await createUserWithEmailAndPassword(auth, email, password)
            const firebase_id = auth.currentUser.uid
            const result = await signup(firebase_id, name, email, status, safeguarding, DEFAULT_PFP)
